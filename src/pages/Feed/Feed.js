...
page:page
  ...
  statusUpdateHandler = event => {
    ...
    const graphqlquery={
      query:`
      mutation UpdateUserStatus($ustatus:String!){
        updatestatus(status:$ustatus){
          status
        }
      }
      `,
      variables:{
        ustatus:this.state.status
      }
    }
    fetch('http://localhost:8080/graphql', {
      method: 'POST',
  ...
  finishEditHandler = postData => {
    ...
    console.log('image ius',postData.image)
    formData.append('image', postData.image);//multer usage
...
fetch('http://localhost:8080/post-image',{
  ...
  const imageUrl=fileresdata.filePath||"undefined"
    console.log('retured image',imageUrl)
  
    console.log(fileresdata)
    // console.log('json',fileresdata.json())
